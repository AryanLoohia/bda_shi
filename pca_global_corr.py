...
def main():
    ...
    # Step 1: Reuse Stored Indegrees
    if os.path.exists(INDEGREE_FILE):
        indegrees = load_existing_indegrees(INDEGREE_FILE)
    else:
        print("Error: Indegree file not found in 'pca_results'. Run indegree calculation first.")
        return

    # Step 2: Load Graph & Interaction Data
    ...
    # Step 3: Weighted Edge Calculation
    ...
    for a, targets in follows_graph.items():
        for b in targets:
            ...
            # S_ab Logic: Prioritizing Replies
    ...
    print(f"Dynamic Threshold Set: {THRESHOLD:.4f}")
    ...
    print("Generating neighbor vectors based on influential clusters...")
    for node in sampled_nodes:
        cluster = get_k_hop_neighbors(node, edge_weights, THRESHOLD, K_HOPS)
        if not cluster: continue

        # Use stored indegrees for neighborhood vector
        ind_vec = sorted([indegrees.get(m, 0) for m in cluster], reverse=True)
        
        # Standardize vector length to TOP_K (20)
    ...
    # Step 5: Correlation PCA (Standardization + PCA)
    if len(data_matrix) > 3:
        print(f"Performing Correlation PCA on {len(data_matrix)} samples...")
        X = np.array(data_matrix)
        
        # Standardize features to mean=0, variance=1 for Correlation-based PCA
        ...
        print("\n" + "="*40)
        print("CORRELATION PCA VARIANCE EXPLAINED")
        print("-" * 40)
        ...
        # Save Output
        output_file = os.path.join(PCA_DIR, "correlation_pca_3d.csv")
        ...
        print(f"PCA complete. Results saved in: {output_file}")
    else:
        print("Insufficient data generated for 3-component PCA.")
